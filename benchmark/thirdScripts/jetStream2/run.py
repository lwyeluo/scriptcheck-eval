...
import shutil
import time
from utils.globalDefinition import _cache_for_Chrome_filepath, _node_run_url_filename_jetstream2, _node_run_url_filename
from utils.globalDefinition import _timeout_for_node_jetstream2_benchmark, _timeout_for_node
...
class RunChromeForPerformance(object):
    def __init__(self, in_url="https://browserbench.org/JetStream/",
        ...
        self.node_filename = _node_run_url_filename_jetstream2
        ...
        self._result_filepath = os.path.join(_dir, "jetstream2_node_results_" + in_type)
    ...
    def run(self):
        ...
        with open(self._result_filepath, "a") as f:
            ...
            r = RunUrl(self.test_url, self.log_path, node_filename=self.node_filename,
                       timeout=_timeout_for_node_jetstream2_benchmark,
                       timeout_for_node=_timeout_for_node_jetstream2_benchmark,
                       chrome_binary=self.chrome_binary)
            if r.flag != CHROME_RUN_FLAG_JETSTREAM2_SUCCESS:
                print(">>> Timeout when running Chrome, retry it later...")
                return False
            f.write(r._results_for_jetsteam2 + "\n\n\n")
            print(r._results_for_jetsteam2)
